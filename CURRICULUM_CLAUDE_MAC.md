...
| Phase 2〜3 | /commit-commands:commit 初体験 | 手動branch + **`/commit-commands:commit`**（メッセージ自動生成）+ 手動push/PR |
| Phase 4〜5 | /commit-commands:commit-push-pr | 手動branch + **`/commit-commands:commit-push-pr`**（add+commit+push+PR一括） |
...
| **Phase 2** | 初期実装 + git統合 + Skills | /commit-commands:commit, バイブコーディング | 3時間 |
...
| 項目 | 習得目標 |
|-----|---------|
| **Skills** | `/commit-commands:commit`, `/commit-commands:commit-push-pr`, `/revise-claude-md` を使いこなし、カスタムSkillを自作できる |
...
5. ~/.claude/ ディレクトリを作成（存在しない場合）:
   ...
   （`/revise-claude-md` コマンドを使えるようにするため。`/commit-commands:commit` は Phase 2、`/feature-dev` は Phase 8 で有効化する）
...
- `/commit-commands:commit` スキル（コミットメッセージ自動生成）
- gh CLI基本操作
...
>
> プラグイン提供カスタムコマンド: `/revise-claude-md`（Phase 0で有効化済み）、`/commit-commands:commit`, `/commit-commands:commit-push-pr`（このフェーズで有効化）、`/feature-dev`（Phase 8で有効化）
...
## タスク P2-4: /commit-commands:commit スキル初体験
...
追加後、Claude Code を再起動して `/commit-commands:commit` が使えることを確認してください。
```

実装が動いたら `/commit-commands:commit` スキルを使ってみる:
...
- [ ] `/commit-commands:commit` スキルを使ってコミットできた

## Phase 2 コミット & プッシュ【/commit-commands:commit スキル + 手動push/PR】
...
```text
/commit-commands:commit      # ← コミットメッセージ自動生成。承認するだけ。
```
...
## Phase 3 コミット & プッシュ【/commit-commands:commit + 手動push/PR（定着）】
...
## タスク P4-4: /commit-commands:commit-push-pr スキル初体験

```
タグフィルタリングの変更を /commit-commands:commit-push-pr スキルでまとめてください。
```
...
- [ ] `/commit-commands:commit-push-pr` でPRを作成できた

## Phase 4 コミット & プッシュ【/commit-commands:commit-push-pr スキル初体験】
...
## Phase 5 コミット & プッシュ【/commit-commands:commit-push-pr（定着）】
...
その後:
1. /commit-commands:commit-push-pr でコミット・プッシュ・PR作成
...
/commit-commands:commit-push-pr スキルで最終PRを作成してください。
```
...
3. /commit-commands:commit-push-pr でPR作成
...
| 習得項目 | 確認方法 |
|---------|---------|
| **Skills** | `/commit-commands:commit`, `/commit-commands:commit-push-pr`, `/revise-claude-md`, カスタム `/taskr-status` |
