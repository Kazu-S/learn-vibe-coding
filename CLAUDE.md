...
| 2026-03-02 | ローカル実行チェック由来のバグ修正（Claude版2ファイル）：`claude --plan`→`--permission-mode plan`・`gh mcp serve`→`npx @modelcontextprotocol/server-github`・P3-1パスハードコード・`__main__.py`未定義・ExitPlanMode表現 (#12, #18) |
| 2026-03-02 | スラッシュコマンド単体ブロックに `text` 言語識別子を付与（Claude版2ファイル）：コピーボタンの視認性向上・プロンプトブロックとの視覚的区別（#13） |
| 2026-03-04 | プラグイン有効化タイミングを修正（Claude版2ファイル）：`commit-commands` を Phase 0→Phase 2、`feature-dev` を Phase 0→Phase 8 に移動（#21） |
